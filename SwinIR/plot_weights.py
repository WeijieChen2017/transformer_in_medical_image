...
# model_best = 50
    ...
    # module = "down1"
        ...
        for target_weight in ["weight", "bias"]:
            for elem in sub_module_hub:
                # print(model_weights[elem].size())
                if target_weight in elem and len(list(model_weights[elem].shape)) > 1:
    ...
    for idx, elem in enumerate(target_hub):
        ...
        # plot
        plt.figure(figsize=(9,6), dpi=300)
        plt.imshow(ele_sim, cmap='bwr', interpolation='nearest', aspect='auto',vmin=-1, vmax=1)
